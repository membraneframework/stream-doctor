defmodule FrameMarker.ReceiverPipeline do
  ...
  Reads an HLS playlist, decodes the stream and restores:
    ...
    * frame numbers from the video bar drawn by `FrameMarker.OverlayFilter`,
    * audio symbol numbers from the marker generated by
      `FrameMarker.AudioMarkerFilter`.

  Options:
    * `:url` - URL of the HLS playlist (`.m3u8`),
    * `:on_frame` - optional callback, see `FrameMarker.DetectorSink`,
    * `:on_audio_symbol` - optional callback, see `FrameMarker.AudioDetectorSink`.
  """
  ...
  defp track_spec(:audio, state) do
    ...
    |> child(:audio_detector, %FrameMarker.AudioDetectorSink{on_symbol: state.on_audio_symbol})
  end
end
