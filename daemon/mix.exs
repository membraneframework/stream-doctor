defmodule StreamDoctor.MixProject do
  ...
  defp releases do
    [
      stream_doctor: [
        steps: [:assemble, &__MODULE__.Symlinks.run/1, &musl_stub/1, &Burrito.wrap/1],
        burrito: [
  ...
  defmodule Symlinks do
      ...
      if File.dir?(build_priv) do
        ...
        |> Enum.map(fn link ->
          ...
          case locate_target(Path.expand(raw, Path.dirname(link)), release) do
            ...
            nil ->
              Mix.raise(
                "#{app}: priv/#{rel_inside_priv} -> #{raw} points outside every app's priv"
              )
          end
        end)
      else
    ...
    @spec locate_target(Path.t(), Mix.Release.t()) ::
            {Application.app(), keyword(), Path.t()} | nil
